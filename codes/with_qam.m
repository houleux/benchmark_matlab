...
encodedeData = ldpcEncode(data,cfgLDPCEnc);
modsignal = qammod(encodedeData, M, InputType="bit");
...
for i = 1 : size(snr, 2)
    ...
    llrout = qamdemod(rxsig, M, OutputType="approxllr", NoiseVariance=noisevar);

    rxbits_flood = ldpcDecode(llrout, cfgLDPCDec, maxnumiter);
    rxbits_layerred = ldpcDecode(llrout, cfgLDPCDec_l, maxnumiter);
    
    ber_flood(i) = nnz(data ~= rxbits_flood)/ndata;
    ber_layered(i) = nnz(data ~= rxbits_layerred)/ndata;
end
 
figure;
plot(snr, ber_flood, 'b-o');      
hold on;
plot(snr, ber_layered, 'r-s');
hold off;
